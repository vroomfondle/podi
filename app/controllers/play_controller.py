...
class PlayController(controller.CementBaseController):
    ...
    def subtitle(self):
        try:
            ...
            self.app.log.error(
                'You must provide a subtitle id number, e.g.: play subtitle 2. Use "inspect player" to see a list of available subtitle streams.')
            return False
        for player in self.app.send_rpc_request(list_active_players()):
            try:
                ...
                if e.error_code == -32602:
                    self.app.log.error(
                        "Kodi returned an 'invalid parameters' error; this stream may not exist? Use \"inspect player\" to see a list of available streams.")
                    return False
                else:
                    raise e
    ...
    def audio(self):
        try:
            ...
            self.app.log.error(
                'You must provide a audio id number, e.g.: play audio 2. Use "inspect player" to see a list of available audio streams.')
            return False
        for player in self.app.send_rpc_request(list_active_players()):
            try:
                self.app.send_rpc_request(
